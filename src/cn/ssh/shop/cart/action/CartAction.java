...
public class CartAction extends ActionSupport implements ModelDriven<Cart>,SessionAware{
	...
	public String add()
	{
		...
		Cart cart1=cartService.findByUid(user.getUid());
		...
		if(cart1==null)
		{
			...
			cartItem.setSubtotal(cartItem.getCount()*product.getShop_price());
		...
		else
		{
			...
			if(cartItem1==null){
				...
				Product product=productService.findByPid(pid);
				int newCount=cartItem1.getCount()+count;
				double newSubtotal=newCount*product.getShop_price();
	...
	public String list()
	{
		...
		cart=cartService.findByUid(user.getUid());
		if(cart!=null){
	...
	public String delete()
	{
		...
		cart=cartService.findByUid(user.getUid());
		double newTotal=cart.getTotal()-subtotal;
		if(newTotal==0){
