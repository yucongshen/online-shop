...
public class FavoriteAction extends ActionSupport implements ModelDriven<Favorite>,SessionAware{
	...
	public String add(){
		...
		if(user==null){
			...
			return "info";
		}else if(favoriteService.findByPidAndUid(pid, user.getUid())!=null){
	...
	{
		User user=(User) session.get("exitUser");
		PageBean<Favorite> pageBeanFavorite=favoriteService.findPageByUid(user.getUid(),page);
	...
	{
		User user=(User) session.get("exitUser");
		favorite=favoriteService.findByPidAndUid(pid,user.getUid());
	...
	{
		User user=(User) session.get("exitUser");
		List<Favorite> list=favoriteService.findByUid(user.getUid());
